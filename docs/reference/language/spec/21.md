...
7. `init()/deinit()`에서는 `self` 리시버를 선언하지 않는다.
8. `init()/deinit() = default;`는 무인자 시그니처만 허용한다.
9. `A(...)`는 class `A`의 `init(...)` 오버로드를 호출하는 생성식이다(v0: `init` 호출 후 `A` 값 반환).
10. dot 호출 `a.m(...)`은 첫 파라미터가 `self`인 메서드만 허용한다.
11. class/proto 멤버 경로 호출 `A::m(...)`, `Proto::m(...)`는 제거되며, 값 dot 호출을 사용한다.
