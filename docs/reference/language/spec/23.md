...
### 11.4.a acts associated type binding

default acts는 proto requirement를 충족하기 위한 associated type binding을 가질 수 있다.
...
1. `type Name = Type;`는 `acts for T` / `acts Name for T` 내부에서만 허용한다.
2. 일반 `acts A {}` 네임스페이스 안에서는 associated type binding을 허용하지 않는다.
3. 같은 acts 블록 안에서 같은 이름의 associated type을 두 번 바인딩하면 에러다.
4. proto 충족 검사는 default acts의 함수와 associated type binding을 함께 본다.
