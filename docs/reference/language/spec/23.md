...
* **struct는 저장소, acts는 행동**이다. (struct/class 내부에 “행동을 강제”하지 않고, 행동은 acts로 분리한다.)
* acts는 다음을 정의한다.
...
* 부착 대상 제한(v0):
  * 허용: `struct`, `class`, `enum`
  * 금지: `actor`, `proto`
* class에 acts를 부착하는 경우에도 class 인스턴스 필드 선언 문법은 `name: Type;` 규칙을 따른다(`let/set` 멤버 문법 없음).

핵심 규칙(v0):

* 타입 `T`의 default acts는 여러 블록으로 분산될 수 있다.
* 각 블록은 같은 owner에 추가 행동을 누적하는 방식으로 합성된다.
...
* 부착 대상 제한은 default acts와 동일하다(`struct`/`class`/`enum` 허용).
...
### 11.4.a acts associated type witness

default acts는 proto requirement를 충족하기 위한 associated type witness를 가질 수 있다.
...
1. associated type witness는 `acts for ...` / `acts Name for ...` 헤더의 `<>` 안에서만 쓴다.
2. `<>`는 binder/witness layer다.
3. `with []`는 이미 소개된 binder에 대한 predicate만 둔다.
4. `is`는 acts owner header 전용 contextual syntax다.
5. `type Name = Type;`는 더 이상 `acts` body에서 허용하지 않는다.
6. 일반 `acts A {}` 네임스페이스에는 associated type witness를 둘 수 없다.
7. 같은 acts 블록에서 같은 associated type 이름을 두 번 witness하면 에러다.
8. proto 충족 검사는 default acts의 함수와 acts-header witness를 함께 본다.
...
3. `self` 표기는 `self` / `mut self` / `self move` 세 형태를 허용한다.
5. 의미:
   - `self`      : `&Self`
   - `mut self`  : `&mut Self`
   - `self move` : `Self`
...
operator(++pre)(mut self) -> T { ... }
operator(++post)(mut self) -> T { ... }
operator(copy)(self) -> T { ... }
operator(clone)(self) -> T { ... }
```
...
5. `copy`/`clone`은 prefix 키워드 연산자이며 `operator(copy)`/`operator(clone)`로만 오버로드할 수 있다.
...
export acts for Foo {
  def inc(mut self) -> void { self.v = self.v + 1u32; }
