...
* `T <: T?` 전역 승격은 허용하지 않는다.
* 단, 대입 경계(let/set 초기화, `=`, struct init, call arg, return, default arg, `??=`)에서는
  `T -> T?` 암시 주입을 허용한다.
...
* `F"..."` 단축 표기는 지원하지 않는다.
* 보간 문법은 `{expr}`만 허용하며 `{{`/`}}`는 리터럴 brace escape다.
* f-string은 컴파일타임 fold를 우선 시도한다.
* 런타임 보간은 문자열 리터럴 빌트인 채널이 아니라 매크로 채널(`$foo"..."`)로 처리한다.
