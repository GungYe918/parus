...
ProtoDecl     := ExportOpt "proto" Ident GenericParamClauseOpt ProtoInheritOpt BlockProto ProtoRequireTailOpt ";" ;
...
ProtoRequireTailOpt := "with" ProtoRequireList | ε ;
ProtoRequireList := "require" "(" Expr ")" ( "," "require" "(" Expr ")" )* ;
...
7. `proto`의 `with require(...)` 목록에서는 trailing comma를 허용하지 않는다.
