...
### 16.9 struct / enum / proto / class / acts
...
```ebnf
CompilerIfItem := "$" "[" "if" "(" Expr ")" "]" ItemDecl ;
...
1. `$[if(...)]`는 **단일 item declaration**만 제어한다(블록 if/else 체인 비지원).
2. `$[if(...)]`의 조건식은 `require(expr)`와 동일한 단순 bool 폴딩(`true/false/not/and/or/==/!=`)만 허용한다.
3. `$![...]`는 현재 단계에서 **파싱/AST 보존만** 수행하며 intrinsic 등록 의미론은 후속 단계에서 확정한다.
4. 매크로 호출 `$path(...)`와 compiler 지시문 `$[...]`, `$![...]`는 구문 계층이 다르며 상호 대체되지 않는다.
