...
* `copy` : **얕은 복사(shallow copy)**, 값의 “비트 단위 복사”에 가까운 복사
  ...
  * POD(struct) 및 기본 수치 타입처럼 “복사 비용과 의미가 명확한 타입”에 적합
...
(단, 기본 수치 타입/단순 struct에 대해서는 컴파일러가 내장 acts를 제공해도 된다.)
...
* 큰 배열/큰 struct를 `copy`로 통째로 복사하려는 경우
