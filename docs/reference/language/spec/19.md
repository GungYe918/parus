...
6. TLS는 thread-local root exception context에만 사용하고, 함수 본문은 explicit throw/call/try/return 경계에서만 예외 상태를 검사한다.
7. `try expr`가 예외를 `null`로 바꾸는 경로에서는 active recoverable payload를 정확히 한 번 정리한다.
8. typed catch가 payload를 바인딩하면 payload는 catch binder로 move되고, 예외 상태는 clear되며 payload 중복 정리는 일어나지 않는다.
9. untyped catch가 outer로 rethrow하지 않고 종료되면 active recoverable payload는 정확히 한 번 정리된다.
9. panic/foreign fault는 ordinary `try...catch`의 포획 대상이 아닌 fatal 채널이다.
10. panic 기본 정책은 `abort/trap`이며, 언와인딩 런타임은 코어 필수가 아니다.
11. C/C++ ABI 경계 바깥으로는 언와인드 전파를 금지한다(경계에서 포획/변환).
12. `Recoverable`와 `Unrecoverable`를 동시에 선언한 payload 타입은 금지한다.
