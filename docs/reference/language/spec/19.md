...
#### (5) local SSA alias는 허용, storage materialization은 계속 금지

v1부터 `~`의 local 표현력은 아래처럼 좁게 확장한다.
...
이때 `set h = ESCAPE_EXPR;`는 ordinary storage 선언이 아니라
**non-addressable local SSA alias binding** 으로 취급한다.

의미:

* alias는 slot을 만들지 않는다.
* alias는 direct arg/return/branch passthrough에만 쓸 수 있다.
* alias는 `materialize_count`를 증가시키지 않는다.
...
* `h.field`
* `arr[i] = h`
* aggregate/field/container initialization with `h`
* non-static assignment target에 `~` 저장
* `struct/class` field type `~T`

즉 Parus는 `~`를 ordinary storable value로 넓히지 않고,
**non-addressable linear value alias**로만 local 표현력을 늘린다.

#### (6) static 예외는 유지한다

기존 static 저장소 예외는 유지한다.
...
즉 top-level static 저장소는 계속 `~`를 담을 수 있다.
다만 이 예외를 일반 local/field/container storage로 일반화하지는 않는다.

#### (7) `materialize_count`의 의미

escape-handle 메타의 `materialize_count`는 더 이상 “local alias 생성”을 뜻하지 않는다.

고정 의미:

* 진짜 storage 진입만 materialization이다.
* local SSA alias는 materialization이 아니다.
