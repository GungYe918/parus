...
4. `?` 함수 ABI는 hidden `exc_ctx*`를 받으며, ordinary direct/imported direct `?` 호출은 current context를 전달한다.
5. TLS는 thread-local root exception context에만 사용하고, 함수 본문은 explicit throw/call/try/return 경계에서만 예외 상태를 검사한다.
6. `try expr`가 예외를 `null`로 바꾸는 경로에서는 active recoverable payload를 정확히 한 번 정리한다.
7. typed catch가 payload를 바인딩하면 payload는 catch binder로 move되고, 예외 상태는 clear되며 payload 중복 정리는 일어나지 않는다.
8. untyped catch가 outer로 rethrow하지 않고 종료되면 active recoverable payload는 정확히 한 번 정리된다.
