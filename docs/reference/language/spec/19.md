...
이 절에서 `~`로 정의되는 전체 의미론과 제약 집합은 문서상 **owner-handle model**이라고 부른다.
...
* raw-pointer/borrow dereference `*place`는 ordinary deref/manual access로만 취급하며, owner transition source에는 포함되지 않는다.
...
`*p` / `*bp`는 계속 ordinary deref/manual gate로만 취급한다.

* deref 결과에서 새 owner extraction을 만들 수 없다.
* deref 결과에서 새 owner borrow를 만들 수 없다.
* owner transition은 계속 root owner place, field/index owner-cell path, optional owner slot, owner-enum payload 경로에서만 일어난다.
...
`~T`는 아래 core places에서 ordinary place type으로 허용된다.
...
* compiler-owned sized array family method surface
  * `arr.swap(i, j)` for any sized array `T[N]`
  * `arr.replace(i, value)` for plain owner array `(~T)[N]`
  * `arr.take(i)`, `arr.put(i, value)` for optional owner array `((~T)?)[N]`
...
* `core::mem::replace` / `core::mem::swap`의 place-first special typing은 `~T`에만 열린다.
...
`~T`는 built-in sized array container에도 저장할 수 있다.
...
sized owner-array는 raw storage syntax를 넘어서
**public container surface**로도 사용한다.

허용되는 compiler-owned method surface:

```parus
pool.items.swap(0usize, 1usize);
let old: ~Worker = pool.items.replace(i, next);

set mut slot = slots.items.take(i);
let h: ~Worker = slot else {
  return null;
};
```

```parus
let old: (~Worker)? = slots.items.put(i, next);
slots.items.swap(0usize, 1usize);
```
...
* sized array family method surface는 const generic/source-level acts가 아니라, 현재는 compiler-owned dispatch로 제공된다.
...
owner-cell path는 local/field/index 한 단계에만 머물지 않고,
...
non-`layout(c)` enum payload에도 owner cell을 담을 수 있다.
...
owner payload가 있는 enum을 `switch`로 꺼낼 때는
...
projected optional owner place에는 `core::mem::take(place)`를 사용할 수 있다.
...
* `core::mem::take`는 projected `(~T)?` place 전용이다.
...
* owner-handle model(`~`)은 recoverable payload 채널과 분리된다.
...
* `catch (e: T)`는 typed payload bind다.
* `catch (e)`는 opaque rethrow token bind다.
  * v0에서는 `throw e`만 보장한다.
  * ordinary 값처럼 읽기/구조 분해/필드 접근은 보장하지 않는다.
...
* 성공 시 `T`, 예외 시 `null`
* `T = ~U`이면 결과 타입은 `(~U)?`다.
* `try expr`는 throwing Parus call에만 적용되며, `extern "C"` / `cimport` 호출에는 사용할 수 없다.
...
1. Recoverable 채널(`throw`/`try...catch`/`try expr`)은 non-unwind 값 채널이다.
2. `? -> ?` 호출은 Recoverable 채널의 암묵 전파를 허용한다.
3. `? -> non-?` 직접 호출은 금지되고 `try expr` 경계를 요구한다.
4. C ABI / `cimport` 호출은 Recoverable source가 아니며 hidden `exc_ctx*`를 받지 않는다.
5. `?` 함수 ABI는 hidden `exc_ctx*`를 받으며, ordinary direct/imported direct `?` 호출과 internal indirect Parus throwing lane은 current context를 같은 ABI로 전달한다.
6. `exc_ctx`는 fixed erased context다.
   * `type_token`: canonical type semantic의 FNV-1a 64-bit hash
   * `payload_size`: concrete payload size
   * `drop_fn`: nullable private drop thunk
   * `payload`: 64-byte inline envelope, align 16
7. Recoverable payload는 fixed envelope(`size <= 64`, `align <= 16`) 안에 들어와야 하며, heap fallback은 없다.
8. TLS는 thread-local root exception context에만 사용하고, 함수 본문은 explicit throw/call/try/return 경계에서만 예외 상태를 검사한다.
9. `try expr`가 예외를 `null`로 바꾸는 경로에서는 active recoverable payload를 정확히 한 번 정리한다.
10. typed catch가 payload를 바인딩하면 payload는 catch binder로 move되고, 예외 상태는 clear되며 payload 중복 정리는 일어나지 않는다.
11. untyped catch가 outer로 rethrow하지 않고 종료되면 active recoverable payload는 정확히 한 번 정리된다.
12. panic/foreign fault는 ordinary `try...catch`의 포획 대상이 아닌 fatal 채널이다.
13. panic 기본 정책은 `abort/trap`이며, 언와인딩 런타임은 코어 필수가 아니다.
14. C/C++ ABI 경계 바깥으로는 언와인드 전파를 금지한다(경계에서 포획/변환).
15. `Recoverable`와 `Unrecoverable`를 동시에 선언한 payload 타입은 금지한다.
16. source-level throwing function type surface와 foreign unwind bridge는 여전히 deferred이며, 현재 indirect throwing lane은 internal ABI foundation으로만 존재한다.
