...
1. `proto`의 `require(...)`에서 사용자 매크로(`$name(...)`)는 지원한다.
1. `require(...)`의 내장 프레디킷 매크로(`$is_plain_data(Self)` 등)는 아직 지원하지 않는다.
1. `require(...)` 파서는 일반 표현식을 수용하지만, v1 타입체커는 단순 bool 식(true/false/not/and/or/==/!=)만 허용한다.
1. `require`의 최종 참/거짓 판정은 proto 선언 시점이 아니라 적용 시점에서 수행한다.
