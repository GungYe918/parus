...
6. actor `init/sub/pub`는 explicit receiver(`self`, `mut self`, `self move`)를 선언할 수 없다.
7. actor 상태 필드 접근은 `draft.x` 표기만 허용한다. `self.x`는 금지한다.
...
6. actor에는 `A { ... }` field-init literal을 사용할 수 없다.
...
actor Counter {
  ...
  def pub bad() -> i32 {
    // 금지: draft move-out
    // set x = ~draft;
    // 금지: self receiver/state access
    // def sub get(self) -> i32 { ... }
    // return self.value;
    // 금지: path call
    // Counter::bad();
    // 금지: brace-init
    // set c = Counter { value: 1i32 };
    commit;
    return 0i32;
  }
}
