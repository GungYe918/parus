...
* 소유권이 있는 값은 move를 기본으로 하되, v1에서는 drop이 필요한 타입만 move-only로 취급한다. trivial builtin/trivial aggregate는 기존처럼 값 복사를 허용한다.
...
* 작은 데이터 전달/공유: trivial 값은 일반 값 전달을 허용하고, drop이 필요한 값은 move-only로 관리한다. 필요 시 copy/clone을 명시한다. (borrow/ref는 v0에서 설계 포함, 구현은 단계적으로)
...
### 0.3 타입 정의 모델: struct, proto, class, acts

* struct: POD 저장소. 생성자/소멸자 없음. 함수 포함 금지.
* proto: 인터페이스(계약). 생성자/소멸자 없음.
* class: 구현체 타입. `init/deinit` lifecycle을 가지며, 필드는 `x: T;` 형태만 허용한다.
* class 상속은 금지한다. 다형성은 `proto` 제약으로 구성한다.
* acts: 특정 타입에 대한 행동(함수, 연산자)을 정의하는 블록. struct 내부 함수 금지 대신 acts로 유도.
