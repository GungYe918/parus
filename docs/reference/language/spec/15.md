...
### 7.6.2 optional place consume-binding

`let/set ... = PLACE else { ... };`는 ordinary optional auto-unwrapping이 아니라, **mutable optional place에서 payload를 소유로 꺼내고 원래 place를 `null`로 되돌리는 special form**이다.

고정 규칙:

* `PLACE`는 assignable mutable place여야 한다.
* `PLACE`의 타입은 반드시 `T?` 형태여야 한다.
* 성공하면:
  * payload가 바인딩 변수에 들어간다.
  * 원래 place는 `null`이 된다.
* 실패하면 `else { ... }`가 실행된다.
* `else` 블록은 fallthrough 없이 종료되어야 한다.
* 일반 expression에는 이 규칙이 적용되지 않는다.
...
이 문법은 optional 편의 문법이면서 동시에, `static T?`/pool entry 같은 저장소에서 값을 꺼내 `return ~v;` 같은 escape boundary로 넘기는 표준 경로다.

---

### 7.6.3 loop 결과 대입 시 타입 강제
