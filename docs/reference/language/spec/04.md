...
* **bundle**: Rust crate 대응 컴파일/배포 단위. `config.lei`을 가진다.
...
* 같은 bundle 안에서도 **다른 파일 선언**을 참조하려면 대상 선언에 `export`가 필요하다.
* bundle 외부로 노출되는 심볼은 `export`로만 결정된다.

### 3.1.1 의존성 선언: bundle plan 우선

의존성은 소스 코드가 아니라 bundle 설정(`config.lei`의 `bundle.deps`)에 선언한다.
...
소스 코드의 `import`는 `deps`에 선언된 의존성 키를 활성화하는 용도다.
...
* `import <head>`의 `<head>`는 현재 bundle `deps`에 반드시 선언되어야 한다.
...
* `use nest`는 namespace alias 전용이다.
* `use nest <Path> [as Alias];`에서 `=`는 금지하고 `as`만 허용한다.
* `use nest <Path>;`에서 alias를 생략하면 마지막 경로 세그먼트를 alias로 사용한다.
* `use nest world::add as w;`처럼 namespace가 아닌 심볼 경로는 금지한다.
...
* 기존 `use path as alias;`는 유지한다. 단 namespace alias 목적이면 `use nest ...`를 권장한다.
...
* `simple item`:
  ...
  * 전역 변수 선언
  * 끝 `;` 필수
* 다른 파일/다른 bundle에서 선언을 참조하려면 선언에 `export`가 필요하다.
