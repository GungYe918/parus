...
* prepass 대상 심볼은 `def/var/struct/acts` 전체다.
...
* `import foo::bar`와 `import ::foo::bar`는 동일 의미이며 내부 canonical은 `foo::bar`다.
* 외부 심볼은 자동 평탄화되지 않는다. 항상 `alias::...`로 접근한다.
* `import <head>`의 `<head>`는 현재 module `imports`에 반드시 선언되어야 한다.
* `module.imports`는 `foo`/`foo::bar`/`::foo::bar` 입력을 허용하고 gate는 top-head(`foo`) 기준으로 canonicalize한다.
...
* `declaration item`: `def`, `struct`, `enum`, `proto`, `class`, `actor`, `acts`, `inst`, 전역 변수
...
* 부동소수: `f32/f64`
* 포인터: `*const T`, `*mut T` (T가 FFI-safe일 때)
* `layout(c)`를 만족하는 `struct`

금지:

* borrow/escape (`&`, `&mut`, `~`)
* optional, actor, class 직접 값 전달
* 구현 의존 내부 타입

### 3.3.2 Parus의 개념이 C에서 어떻게 대응되는가

* `struct` -> `struct` (C 호환 레이아웃)
* `handle<T>` -> `void*`(또는 `struct Handle_T*`) + 생성/해제 함수 세트
* `class` -> C에서 직접 값으로는 못 다루고, 반드시 `handle<class>`로만 다룬다
* `actor`/`draft`/`pub-sub` -> **FFI로 직접 노출하지 않는다**
  대신:
