...
6. 허용: `acts for Range<T is Item, iter::RangeIter<T> is Iter> with [T: Proto] { ... }`
7. 금지: `acts for Vec<T> <T> { ... }`
8. `acts for ...`의 `<>`는 binder/witness layer다.
9. `with []`는 constraint layer다.
10. 따라서 `with [T: Proto]`, `with [T == U]`는 허용하지만 `with [T is Item]`은 금지한다.
11. `enum` 선언도 제네릭 파라미터를 가질 수 있다.
