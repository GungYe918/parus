...
2. Unrecoverable 채널 (panic/foreign fault)
   * 스택 언와인딩 기반 채널이다.
   * payload는 `Unrecoverable` proto를 만족하는 값으로 분류한다.
   * 런타임/FFI 경계에서 포획 후 종료 또는 변환한다.
3. 언와인딩 런타임은 코어 필수다.
4. Recoverable/Unrecoverable 두 채널 모두 정리 구간에서는 `deinit`가 `return`과 동일한 LIFO 순서로 수행된다.
