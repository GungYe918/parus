...
이 문서는 `docs/reference/abi/v0.0.1/ABI.md`의 문자열 관련 요약을 상세화한다.
충돌 처리 우선순위는 다음과 같다.

1. `docs/reference/abi/v0.0.1/ABI.md`
2. 본 문서
3. `docs/reference/language/SPEC.md`
...
4. `F"..."` 단축 표기는 비허용이며, `F"""..."""` 또는 `$"..."`를 사용해야 한다.
5. 보간 규칙은 `{expr}`만 허용하며, `{{`/`}}`는 리터럴 brace escape로 취급한다.
6. f-string은 컴파일타임 fold를 우선 시도한다.
7. fold가 실패한 런타임 경로는 v1에서 `$"{text_expr}"` 단일 보간 형태만 허용한다.
