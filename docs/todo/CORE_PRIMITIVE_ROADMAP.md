...
3. `ptr/borrow` utility surface는 memory contract 없이 무리해서 더 넓히지 않는다
4. `float` total ordering은 bit-level lowering contract가 먼저 필요하다
5. iterator는 `core`에 두되, generic/borrow/loop protocol 안정화 이후에 도입한다
