...
4. 현재 직접 구현 우선순위는 source-level proto import ergonomics와 monomorphization quality 정리다.
...
1. source-level proto target qualified path import ergonomics를 연다
2. canonical mono key helper와 sidecar dedup을 정리한다
3. helper `actor` dependency closure는 actor 의미론 고정 전까지 계속 보류한다
4. installed/external lane에서 imported metadata canonical resolution과 source-level proto target resolution을 더 정교하게 맞춘다
