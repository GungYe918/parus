...
| `core::convert` | `Into<T>`, `AsRef<T>`, `AsMut<T>`, free `into` helper | generic proto-instantiated constraint syntax, borrow-return helper support, concrete impl inventory, fallible conversion |
...
13. `core::convert` contract-first surface (`Into/AsRef/AsMut` + free `into` helper)
...
11. `core::convert` v1은 contract-first로 고정한다
   - `Into<T>`, `AsRef<T>`, `AsMut<T>`와 free `into` helper를 제공한다
   - current grammar에서는 `with [U: Into<T>]` 같은 generic proto-instantiated constraint를 아직 못 써서 `into` helper는 unconstrained wrapper로 둔다
   - ordinary function body에서 borrow를 반환할 수 없기 때문에 free `as_ref/as_mut` helper는 후속 라운드로 미룬다
   - installed/external bundle 경계에서 user-defined owner에 대한 generic helper instantiation은 아직 완전히 고정되지 않았으므로, runtime smoke는 dot-call도 같이 유지한다
