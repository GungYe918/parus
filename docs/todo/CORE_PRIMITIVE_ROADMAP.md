...
6. 장기적인 compiler query / metaprogramming 확장은 `inst`가 맡고, immediate ABI/layout surface는 `Impl::*`가 맡는다.
...
3. `core::mem::{size_of, align_of}`는 source body를 두지 않고 `Impl::SizeOf`, `Impl::AlignOf` attached binding으로 선언한다
4. `core::hint::spin_loop`는 source body를 두지 않고 `Impl::SpinLoop` attached binding으로 선언한다
5. `core::hint::unreachable`는 checked trap/unchecked UB 의미가 아직 고정되지 않았으므로 bootstrap 단계에서 fake body를 유지한다
6. `ptr/borrow` utility surface는 memory contract 없이 무리해서 더 넓히지 않는다
7. `float` total ordering은 bit-level lowering contract가 먼저 필요하다
8. iterator는 `core`에 두되, generic/borrow/loop protocol 안정화 이후에 도입한다
