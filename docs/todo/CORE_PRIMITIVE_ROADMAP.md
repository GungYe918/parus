...
| `text` | `is_empty`, `len_bytes` | prefix/suffix, ASCII-insensitive compare, bytes/iteration, indexing/slicing |
...
5. `char` ASCII helper
6. `text` 최소 view acts (`is_empty`, `len_bytes`)
...
1. `text` 최소 view surface는 이제 언어 코어 확장 위에서 구현되었다
   ...
   - `text{ data, len }`
2. `ptr/borrow` utility surface는 memory contract 없이 무리해서 더 넓히지 않는다
