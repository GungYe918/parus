...
1. external generic field/enum/type body dependency closure를 free-function-only에서 넓히는 작업
2. generic actor lane을 common monomorphization service로 옮기는 작업
3. external generic type body import에서 field/enum/type body dependency를 activation할 범위 결정
4. installed/external lane에서 source-level explicit import 완화 없이도 imported metadata가 더 넓게 동작하도록 canonical resolution을 확장
