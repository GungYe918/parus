...
12. 예외 코어는 non-unwind Recoverable 채널로 고정되었다
   ...
   - `exc_ctx`는 canonical semantic token + 64-byte inline envelope(align 16)를 쓰는 fixed erased context다
   - same erased `exc_ctx*` ABI는 internal indirect Parus throwing / callback / future dyn thunk foundation에도 공통으로 적용된다
