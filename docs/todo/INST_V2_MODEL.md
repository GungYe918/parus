...
1. `Impl::*`
   ...
   - body 없음 + recognized key는 compiler-owned impl point다
   - body 있음은 library-owned implementation이며 compiler special-case 대상이 아니다
2. `inst`
...
2. recognized `Impl::*` + body 없음일 때만 compiler가 의미를 구현한다
3. 같은 `Impl::*` key라도 body가 있으면 ordinary library implementation으로 취급한다
4. fake body는 두지 않는다
