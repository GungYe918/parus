...
현재는 `Ordering` helper와 generic `min/max/clamp`까지 1차 surface로 본다.
...
10. `to_le/be/ne`
11. `from_le/be/ne`
이 함수들은 언어/백엔드 intrinsic contract를 먼저 정해야 한다. `pow`는 pure Parus helper로 먼저 구현 가능한 표면이므로 1차로 끌어올린다.
...
7. `len_utf8(self) -> usize`

2차 메서드:

1. `to_digit(self, radix: u32) -> u32?`
...
Parus에는 이제 `core::slice` free-function foundation이 있다. 다만 `acts for T[]`나 text byte-view bridge까지 같이 열면 generic/borrow/iterator 부담이 커진다.
