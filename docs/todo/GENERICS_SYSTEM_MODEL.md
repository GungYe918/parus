...
v1 코어 위에 아래 안정화 규칙을 추가 적용한다.

1. 제네릭 named type(`Foo<...>`)는 문자열 split이 아니라 구조적 표현(`path + arg slice`)을 기본으로 사용한다.
1. `run_on_program` 이후 `TypeChecker` 이전에 `GenericPrep`를 실행한다.
1. `GenericPrep`는 템플릿 인덱스(종류/arity)와 조기 검증 정보를 만든다.
1. 타입체커는 제네릭 경로 오류를 단일 핵심진단으로 우선 보고하고, 2차 파생 진단은 억제한다.
1. 파서의 `<...>` 추측/롤백 경로는 공통 스캐너를 사용해 call/literal/type-path 동작을 맞춘다.
1. 맹글/ABI 문자열 출력 포맷은 기존과 호환된다.

지원 경계(v1.1):

1. 지원: `function/class/proto/acts/struct` 제네릭(정적 모노모피제이션)
1. 비지원: `actor` 선언 제네릭, `dyn` 결합, 런타임 모노모피제이션
