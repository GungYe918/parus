...
5. module별 import gate는 `module.imports`로 선언한다(입력 `foo`, `foo::bar`, `::foo::bar` 허용).
...
9. `module.head` 사용자 입력은 제거되었고, 엔진이 `module.sources`에서 자동 계산한다.

## Parus interop 계약

1. 같은 module(같은 폴더) 내부 auto-share는 `export` 선언만 허용한다.
2. 다른 module 참조는 `import <head> as <alias>;`가 필요하다.
3. Parus `import`는 `import foo::bar as x;`와 `import ::foo::bar as x;`를 모두 허용한다.
4. `import <head>`의 `<head>`는 현재 module의 `module.imports`에 있어야 한다.
5. 내부 gate는 top-head 기준으로 canonicalize되어 검증된다.
6. cross-bundle import가 발생하면 대상 bundle은 `bundle.deps`에도 있어야 한다.
7. `nest`는 네임스페이스 태깅 전용이며 module/import head 계산에 관여하지 않는다.
